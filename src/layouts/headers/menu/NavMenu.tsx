      ...
      <ul className="navigation">
         {menu_data.map((menu) => (
            <li key={menu.id} className={
               Array.isArray(menu.home_sub_menu) && menu.home_sub_menu.length > 1 ? "menu-item-has-children" : ''}>

               <NavLink
                  className={({ isActive }: { isActive: boolean }) =>
                     isActive ? 'active' : ''
                  }
                  onClick={() => setNavClick(!navClick)} to={menu.link}>
                  {menu.title}
               </NavLink>
               {Array.isArray(menu.home_sub_menu) && menu.home_sub_menu.length > 1 && <ul className={`sub-menu ${menu.menu_class}`}>
                  {menu.home_sub_menu ? (
                     <>
